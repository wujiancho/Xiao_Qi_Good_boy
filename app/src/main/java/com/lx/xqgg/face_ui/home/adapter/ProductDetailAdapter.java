...
public class ProductDetailAdapter extends BaseQuickAdapter<FaceProductBean.ProductBean, BaseViewHolder> {
    public ProductDetailAdapter(@Nullable List<FaceProductBean.ProductBean> data) {
        super(R.layout.item_face_product, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, FaceProductBean.ProductBean item) {
        Glide.with(mContext)
                .load(Config.IMGURL + item.getSeriesImage())
                .apply(new RequestOptions().placeholder(R.drawable.ic_default))
                .into((ImageView) helper.getView(R.id.imageView));
        helper.setText(R.id.tv_title, item.getSeriesTitle());
    }
}
